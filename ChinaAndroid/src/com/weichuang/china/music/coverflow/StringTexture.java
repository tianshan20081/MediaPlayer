...
public final class StringTexture extends Texture {
    ...
    public static int computeTextWidthForConfig(float textSize, Typeface typeface, String string) {
        ...
        synchronized (paint) {
            ...
            return (int) (10.0f * 1.0f) + (int) FloatMath.ceil(paint.measureText(string));
        }
    }
    ...
    @Override
    protected Bitmap load(CoverFlow view) {
        if (mString == null)
            return null;
        Paint paint = computePaint();
        String stringToDraw = mString;
        ...
        if (config.sizeMode == Config.SIZE_BOUNDS_TO_TEXT || config.sizeMode == Config.SIZE_EXACT) {
            // do something else
        ...
        if(backWidth >256){
        	stringToDraw = TextUtils.ellipsize(mString, (TextPaint) paint, 256-2 * padding, TextUtils.TruncateAt.END).toString();
        	paint.getTextBounds(stringToDraw, 0, stringToDraw.length(), bounds);
        	backWidth = bounds.width() + 2 * padding;
        }
