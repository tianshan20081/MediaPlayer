 ...
 */
public class CoverFlowScroller  {
    private int mMode;
     ...
     */
    public CoverFlowScroller(Context context) {
        this(context, null);
    }
     ...
     */
    public CoverFlowScroller(Context context, Interpolator interpolator) {
        mFinished = true;
    ...
    public void fling(int startX, int startY, int velocityX, int velocityY,
            int minX, int maxX, int minY, int maxY,int step,int distanceToNextStep) {
        mMode = FLING_MODE;
        mFinished = false;

        float velocity = (float)Math.hypot(velocityX, velocityY);
        if(velocityX > 0){
        	distanceToNextStep = step-distanceToNextStep;
        }
        float totalDistance = ((velocity * velocity) / (2 * mDeceleration));
        totalDistance = (float)Math.floor((totalDistance/step))*step + distanceToNextStep;
        mDuration = (int)(1000*Math.sqrt(totalDistance*2/mDeceleration));
        mVelocity = mDuration*mDeceleration/1000;
        if(velocityX < 0){
...
//        mDuration = (int) (1000 * velocity / mDeceleration); // Duration is in
        ...
        mFinalX = startX + Math.round(totalDistance);
